...
def pick_chunk(session_id: str):
    if not session_id in db:
        return HTTPException(status.HTTP_404_NOT_FOUND, f"session {session_id} doesn't exist")
    session = db[session_id]
    if session.can_pick():
        session.pick()
    else:
        return HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR, "ran out of files to pick amongst")
...
def get_session(session_id: str, direction:str="above"):
    ...
    if session.can_peek():
        if direction == "above":
            ...
            return HTTPException(status.HTTP_400_BAD_REQUEST, "direction query parameter can only be 'above' or 'below'")
    else:
        return HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR, "unable to call peek")
