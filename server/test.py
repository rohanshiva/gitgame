...
while True:
    ...
    if keyboard_input.upper() == "W":
        print(requests.get(f"http://localhost:8000/session/{session_id}/peek?direction=above").json())
        dump_chunk(session_id)
    elif keyboard_input.upper() == "S":
        print(requests.get(f"http://localhost:8000/session/{session_id}/peek?direction=below").json())
        dump_chunk(session_id)
