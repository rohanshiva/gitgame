...
def chunk_fetcher_factory(file: File) -> ChunkFetcher:
    return WindowChunkFetcher(file, starting_chunk_size=STARTING_CHUNK_SIZE, peek_size=PEEK_SIZE, remaining_peeks=REMAINING_PEEKS)

def get_mock_file(readlines_callback: Callable[[], List[str]], file_name="sample.py", file_path="./sample.py", user="User1", repo="Repo1") -> Mock:
...
def test_fileLinesLargerThanStartingChunkSize_shouldPickMostSignificantChunk():
    ...
    mock_file = get_mock_file(readlines_callback, file_name="index.js", file_path=test_file_path)
    chunk_fetcher = chunk_fetcher_factory(mock_file)
    chunk_fetcher.pick_starting_chunk()
...
def test_peekOnFile_peekShouldEnlargeChunkBasedOnDirection():
    ...
    mock_file = get_mock_file(readlines_callback, file_name="main.py", file_path=test_file_path)
    chunk_fetcher = chunk_fetcher_factory(mock_file)
    chunk_fetcher.pick_starting_chunk()
    ...
    expected_chunk_lines = test_file_lines[7 - PEEK_SIZE: 7 + STARTING_CHUNK_SIZE + PEEK_SIZE]
    assert actual_chunk_lines == expected_chunk_lines
